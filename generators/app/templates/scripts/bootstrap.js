...
(function () {
  ...
  function logRejectedPromises() {
    unhandledRejections.forEach(function (reason) {
      <%= projectNameUpperCase %>.Analytics.trackError('UnhandledPromiseRejection', reason);
    });
  ...
  function lazyLoadWCPolyfillsIfNecessary(callback) {
    ...
    if (!(<%= projectNameUpperCase %>.Util.getChromeVersion() &&
      <%= projectNameUpperCase %>.Util.getChromeVersion() >= 46 || <%= projectNameUpperCase %>.Util.getFirefoxVersion() && <%= projectNameUpperCase %>.Util.getFirefoxVersion() >= 40)) {
      var parent = document.querySelector('body');
  ...
  window.addEventListener('offline', function () {
    if (<%= projectNameUpperCase %>.Elements && <%= projectNameUpperCase %>.Elements.Template.$.toast) {
      <%= projectNameUpperCase %>.Elements.Template.$.toast.show(
  ...
  window.addEventListener('beforeinstallprompt', function (event) {
    <%= projectNameUpperCase %>.Analytics.trackEvent('installprompt', 'fired');

    event.userChoice.then(function (choiceResult) {
      // choiceResult.outcome will be 'accepted' or 'dismissed'.
      // choiceResult.platform will be 'web' or 'android' if the prompt was
      // accepted, or '' if the prompt was dismissed.
      <%= projectNameUpperCase %>.Analytics.trackEvent('installprompt', choiceResult.outcome,
  ...
  function initApp() {
    ...
    lazyLoadWCPolyfillsIfNecessary(function () {
      <%= projectNameUpperCase %>.Elements.init();
    });
  }
